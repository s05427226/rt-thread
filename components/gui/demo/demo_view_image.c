...
static void open_btn_onbutton(rtgui_widget_t *widget, struct rtgui_event *event)
{
    ...
    rtgui_widget_show(RTGUI_WIDGET(filelist));
    {
...
static rt_bool_t demo_view_event_handler(rtgui_widget_t *widget, rtgui_event_t *event)
{
    ...
    result = rtgui_container_event_handler(RTGUI_OBJECT(widget), event);
...
rtgui_container_t *demo_view_image(void)
{
    ...
    if (_container != RT_NULL)
        ...
        rtgui_object_set_event_handler(RTGUI_OBJECT(_container), demo_view_event_handler);
